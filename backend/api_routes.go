...
func SetupAPIRoutes(api fiber.Router) {
	...
	// Forward input events over HTTP (fallback when WebSocket input is disabled)
	api.Post("/sessions/:id/input", func(c fiber.Ctx) error {
		sessionID := c.Params("id")
		...
		if err := json.Unmarshal(body, &input); err == nil && input.Type != "" {
			handleInputEvent(cli, containerInfo.ID, input)
			return c.JSON(fiber.Map{"ok": true})
		}

		var gamepad GamepadEvent
		if err := json.Unmarshal(body, &gamepad); err == nil && gamepad.Type != "" {
			handleGamepadEvent(cli, containerInfo.ID, gamepad)
			return c.JSON(fiber.Map{"ok": true})
		}

		return c.Status(400).JSON(fiber.Map{"error": "Invalid input payload"})
	})
	...
	api.Get("/games", func(c fiber.Ctx) error {
		games := []fiber.Map{
			...
			},
			{
				"id":          "freeciv",
				"name":        "Freeciv",
				"description": "Turn-based strategy (server + GTK client)",
