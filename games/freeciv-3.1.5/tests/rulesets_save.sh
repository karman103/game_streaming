...
if test x$KEEP_RESULT_IN_DIR = x ; then
...
if [ -d "${tmpdir}" ] ; then
  cp -R /Users/karman/Desktop/projects/mvp/freeciv-3.1.5/data/ruledit/ "${tmpdir}" || exit 1
  cp -R /Users/karman/Desktop/projects/mvp/freeciv-3.1.5/data/default/ "${tmpdir}" || exit 1
else
...
for ruleset in $rulesets; do
  ...
  /Users/karman/Desktop/projects/mvp/freeciv-3.1.5/fcruleup -F -c -r $ruleset \
      --output "$resaved_path" \
    || exit 1

  echo "Trying to load $ruleset saved as $resaved"
  FREECIV_DATA_PATH=$tmpdir \
      /Users/karman/Desktop/projects/mvp/freeciv-3.1.5/tests/rs_test_res/ruleset_loads.sh "$resaved" \
    || exit 1

  echo "Checking ruleset saving stability for $ruleset"

  echo "Resaving resaved $ruleset"
  FREECIV_DATA_PATH=$tmpdir /Users/karman/Desktop/projects/mvp/freeciv-3.1.5/fcruleup -F -c -r "$resaved" \
